          ...
          <div className={
            ...
            {filteredProducts.map((product) => {
              ...
              return (
                <div
                  key={product.id}
                  className="rounded-xl shadow-sm hover:shadow-md transition-all cursor-pointer overflow-hidden flex items-center gap-4 p-4 pl-1"
                  onClick={() => setSelectedProduct(product)}
                  style={{
                    borderRadius: theme.button_style === 'rounded' ? '0.75rem' : '0.25rem',
                    backgroundColor: cardBackgroundColor
                ...
                >
                  {product.images[0] && (
                    <img
                      src={product.images[0]}
                      alt={product.name}
                      className="w-full h-full object-cover rounded-xl flex-shrink-0 " /*DF: agregue el rounded-xl para que se vea cuadrada la imagen*/
                    />
                  )}
